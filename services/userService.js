...
module.exports = {
  ...
  createUser: async data => {
    ...
    if (created) {
      var result = await userModel.findOne("key", uniqid);
      if (result != "") {
        var updated = await userModel.updateRegister(uniqid);
        if (updated)
          return{ message: "User created with success", status: 200 };
        else return { message: "couldn't update status", status: 400 };
      } else return { message: "couldn't update status", status: 400 };


     /*
      var link = "https://localhost:3000/users/register/" + uniqid;
      await sendmail.registerMail(data[1], data[0], link);
